...
function Playlyfe(options) {
  ...
  this._oauth2 = new OAuth2(options.clientID, options.clientSecret, 'http://playlyfe.com', '/auth', '/auth/token', options.customHeaders);
}
...
Playlyfe.prototype._request = function(method, url, headers, data, accessToken, refreshToken, callback) {
  ...
  this._oauth2._request(method, url, headers, data, accessToken, function(err, result, response) {
    ...
    if(err && self.options.redirectURI != null) {
      try {
        ...
        if( err.data.error === 'invalid_access_token' && refreshToken) {
          ...
          self._oauth2.getOAuthAccessToken(
            ...
            function(err, newAccessToken, newRefreshToken, results) {
              if (err) {
                self.clearTokens(self.options.clientID)
                return callback(err);
              } else {
                // Store new access token
                self.storeTokens(self.options.clientID, newAccessToken, newRefreshToken)
      ...
      self._oauth2.getOAuthAccessToken(
        '',
        { client_id: self.options.clientID,  client_secret: self.options.clientSecret, grant_type: 'client_credentials' },
        function(err, newAccessToken) {
          if (err) {
            self.clearTokens(self.options.clientID)
            return callback(err);
          } else {
            self.storeTokens(self.options.clientID, newAccessToken)
            // Retry with new access token
            self._oauth2._request(method, url, headers, data, newAccessToken, function(err, result, response) {
              if(err) {
                try{
...
Playlyfe.prototype.api = function(url, method, data, accessToken, refreshToken, callback) {
  ...
  if(method === 'GET' || method === 'DELETE') {
    data = null;
    self._request(method, 'http://api.playlyfe.com/' + this.options.version + url, headers, data, accessToken, refreshToken, callback);
  } else {
    try {
...
Playlyfe.prototype.utils = function(req, res) {
  ...
  return {
    ...
    api: function(url, method, data, callback) {
      var tokens = self.getTokens(self.options.clientID);
