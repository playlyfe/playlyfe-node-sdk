...
var Playlyfe, PlaylyfeException, Promise, _, jwt, request,
...
Playlyfe = (function() {
  Playlyfe.createJWT = function(options) {
    var client_id, client_secret, expires, payload, player_id, scopes, token;
    client_id = options.client_id, client_secret = options.client_secret, player_id = options.player_id, scopes = options.scopes, expires = options.expires;
    if (scopes == null) {
      scopes = [];
    }
    if (expires == null) {
  ...
  function Playlyfe(options1) {
    ...
    if ((base = this.options).auth_endpoint == null) {
      base.auth_endpoint = "https://playlyfe.com";
    }
    if ((base1 = this.options).api_endpoint == null) {
      base1.api_endpoint = "https://api.playlyfe.com";
    }
    if ((base2 = this.options).strictSSL == null) {
      base2.strictSSL = true;
    }
    if ((base3 = this.options).store == null) {
      base3.store = (function(_this) {
        return function(access_token, done) {
          _this.access_token = access_token;
          return done(null, _this.access_token);
        };
      })(this);
    }
    if ((base4 = this.options).load == null) {
      base4.load = (function(_this) {
  ...
  Playlyfe.prototype._done = function(err, result) {
    if (err) {
  ...
  Playlyfe.prototype.getAuthorizationURI = function() {
    return this.options.auth_endpoint + "/auth?" + (require("querystring").stringify({
      response_type: 'code',
  ...
  Playlyfe.prototype.makeRequest = function(method, url, query, body, full_response) {
    var data;
        ...
        if ((err.response != null) && /application\/json/.test(err.response.headers['content-type'])) {
          ...
          if (res_body.error === 'invalid_access_token') {
            return _this.getAccessToken().then(function() {
              return _this.api(method, url.replace(_this.endpoint, ''), query, body, full_response);
            });
          } else {
  ...
  Playlyfe.prototype.makeTokenRequest = function(body) {
    return this.makeRequest('POST', this.options.auth_endpoint + "/auth/token", {}, body).then((function(_this) {
      return function(token) {
        token.expires_at = new Date(new Date().getTime() + (parseInt(token.expires_in) * 1000));
        return _this.options.store(token, _this._done).then(function() {
  ...
  Playlyfe.prototype.getAccessToken = function(code) {
    ...
    if (this.options.type === 'code') {
      return this.options.load(this._done).then((function(_this) {
        return function(token) {
          if (token != null) {
  ...
  Playlyfe.prototype.api = function(method, url, query, body, full_response) {
    if (query == null) {
      query = {};
    }
    if (body == null) {
  ...
  Playlyfe.prototype.get = function(url, query, full_response) {
    return this.api('GET', url, query, null, full_response);
  };

  Playlyfe.prototype.post = function(url, query, body, full_response) {
    return this.api('POST', url, query, body, full_response);
  };

  Playlyfe.prototype.patch = function(url, query, body, full_response) {
    return this.api('PATCH', url, query, body, full_response);
  };

  Playlyfe.prototype.put = function(url, query, body, full_response) {
    return this.api('PUT', url, query, body, full_response);
  };

  Playlyfe.prototype["delete"] = function(url, query, full_response) {
    return this.api('DELETE', url, query, null, full_response);
  };

  Playlyfe.prototype.upload = function(url, query, formData, full_response) {
    var data;
    data = {
